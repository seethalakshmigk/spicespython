...
def add_cart(request, product_id):
    ...
    try:
        ...
        cart_key = _cart_id(request)
        cart = Cart(cart_id=cart_key)  # Create a new cart object
        cart.save()

    try:
        cart_item = Cart_Item.objects.get(product=product, cart=cart)
        if cart_item.quantity < cart_item.product.stock:
            cart_item.quantity += 1  # Use lowercase 'quantity'
            cart_item.save()
...
def cart_details(request, total=0, counter=0, cart_item=None):
    try:
        ...
        for cart_item in cart_items:
            total += cart_item.product.price * cart_item.quantity  # Corrected line
